...
class SIFA:
    ...
    def __init__(self, data_dir, tensorboard_dir, checkpoints_dir, save_model_dir, sample_dir,
        ...
        self.template = "{4}/{5} in {6:.4f} sec - S_d_loss: {0:.5f} - T_d_loss: {1:.5f} - S2T_g_loss: {2:.5f} - T2S_g_loss: {3:.5f} - DiceLoss: {7:.5f} - DiceCoeff: {8:.5f}"
    ...
    def _train_segmentation(self, T, S2T, S_seg, T_g_loss):
        """
       Segmentation training consists of the following steps:
       1) segmentation for T: segmentation(encoder(T) = T_segm
       2) segmentation for S2T: segmentation(encoder(S2T)) = S2T_segm
       3) loss calculation:
    ...
    def train_step(self, S, T, S_seg, step, D_loss_dict, epoch, segm_epoch):
        """
        One training step:
        1) train generators
        ...
        if epoch > segm_epoch:
            ...
            T_segm, S2T_segm, S_loss_dict = self._train_segmentation(T, S2T, S_seg, T_g_loss)
        else:
    ...
    def train(self, epochs0=0, epochs=50, segm_epoch=50, data_nr=None, restore=True, step_decay=None):
        ...
        for epoch in range(epochs0, epochs + 1):
            ...
            for idx in range(data_nr):
                ...
                start = time()
                # train step
                G_loss_dict, D_loss_dict, S_loss_dict = self.train_step(S, T, S_seg, idx, D_loss_dict, epoch,
                                                                        segm_epoch)
                elapsed = time() - start
